...
block TablurGearR2T
  ...
  parameter Modelica.Blocks.Types.Smoothness smoothness=Modelica.Blocks.Types.Smoothness.MonotoneContinuousDerivative1
...
protected
  ...
  Modelica.SIunits.Angle da;
  Modelica.SIunits.Position ds;
...
equation
  ...
  da = flangeR.phi - internalSupportR.phi;
  ds = flangeT.s - internalSupportT.s;

  // Force equilibrium
  0 = flangeR.tau + der(ds) * flangeT.f;
  ...
  if smoothness == Modelica.Blocks.Types.Smoothness.ConstantSegments then
      ds = Modelica.Blocks.Tables.Internal.getTable1DValueNoDer(tableID, 1, da);
  else
      ds = Modelica.Blocks.Tables.Internal.getTable1DValue(tableID, 1, da);
  end if;
